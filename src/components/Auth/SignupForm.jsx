...
export default function SignupForm({ onSwitch }) {
  ...
  if (success) return (
    <div className="text-center space-y-3">
      <p className="text-[0.82rem] text-df-text2 leading-relaxed">
        Check your email to confirm your account, then sign in.
      </p>
      <button onClick={onSwitch} className="text-[0.72rem] text-df-accent hover:underline">
        Back to sign in
      </button>
    </div>
  )
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <p className="text-center text-[0.72rem] text-df-text3">
        Have an account?{' '}
        <button type="button" onClick={onSwitch} className="text-df-accent hover:underline">Sign in</button>
      </p>
    </form>
  )
}
